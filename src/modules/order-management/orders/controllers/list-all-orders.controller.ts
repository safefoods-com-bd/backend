...
/**
 * Retrieves all orders with pagination, including order history for each order
 * @param req Express request object
 * @param res Express response object
 * @returns JSON response with orders, their history, or error message
 */
export const getAllOrdersV100 = async (req: Request, res: Response) => {
  try {
    ...
    const ordersQuery = db
      .select({
        ...
        guestUserId: ordersTable.guestUserId,
        ...
        guestUser: {
          ...
          fullName: guestUsersTable.fullName,
          email: guestUsersTable.email,
          phoneNumber: guestUsersTable.phoneNumber,
        },
      })
      .from(ordersTable)
      ...
      .leftJoin(
        guestUsersTable,
        eq(ordersTable.guestUserId, guestUsersTable.id),
      )
    ...
    handleError(error, res, ORDER_ENDPOINTS.GET_ALL_ORDERS);
  }
};
